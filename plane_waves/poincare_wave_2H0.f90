program poincare_wave
    ...
    namelist /param_fisicos/ f0, L_x, L_y, amp, g, H
    ...
    open(unit=10, file='input_2H0.nml', status='old', action='read')
    ...
    open(unit=20, file='poincare_data_2H0.txt', status='unknown')
    ...
    do i = 0, N-1
        ...
        do j = 0, N-1
            ...
            u = (amp * g / (omega**2 - f0**2)) * (k * omega * cos(k*x + l*y))
            v = (amp * g / (omega**2 - f0**2)) * (l * f0 * sin(k*x + l*y))

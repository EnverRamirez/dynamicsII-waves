...
# Se os dados incluirem U e V, descomente as linhas abaixo
...
try:
    ...
    # η = amp * cos(kx + ly)
    # u = (g/omega) * [ (k/omega) * ∂η/∂t - (f0/omega) * ∂η/∂y ]
    # v = (g/omega) * [ (l/omega) * ∂η/∂t + (f0/omega) * ∂η/∂x ]
    # Para t=0: ∂η/∂t = 0, ∂η/∂x = -amp*k*sin(kx+ly), ∂η/∂y = -amp*l*sin(kx+ly)
    
    fase = k * X + l * Y
    U = (g * f0 / (omega**2 - f0**2)) * (l * np.sin(fase))  # Aproximação
    V = -(g * f0 / (omega**2 - f0**2)) * (k * np.sin(fase))
    
    # Versão mais precisa com desfase de 90 graus
...
# Normalizar os vetores para melhor visualização (opcional)
magnitude = np.sqrt(U_sub**2 + V_sub**2)
U_norm = U_sub / (magnitude + 1e-10)
V_norm = V_sub / (magnitude + 1e-10)
...
quiver = ax2.quiver(X_sub, Y_sub, U_sub, V_sub, magnitude[::step, ::step],

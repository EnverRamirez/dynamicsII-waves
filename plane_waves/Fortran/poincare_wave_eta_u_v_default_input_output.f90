program poincare_wave
    implicit none

    ! Variables para nombre de los archivos
    character(len=256) :: input_namelist, output_txt_file

    ! Variables del Namelist
    real :: f0, L_x, L_y, amp, g, H
    namelist /param_fisicos/ f0, L_x, L_y, amp, g, H, output_txt_file

    ! Variables de cálculo
    real :: k, l, omega, c
    real :: x, y, eta, u, v, u_par, u_per
    integer :: i, j, arg_count
    integer, parameter :: N = 100 ! Resolución de la grilla (NxN)
    real :: x_max, y_max, dx, dy
   
    arg_count = command_argument_count()
    if ( arg_count == 1 ) then
          call get_command_argument(1, input_namelist)
    else
          input_namelist = 'input.nml'
    end if

    output_txt_file = 'default_output.txt'

    ! 1. Leer parámetros desde el Namelist
    open(unit=10, file=trim(input_namelist), status='old', action='read')
    read(10, nml=param_fisicos)
    close(10)

    print *, "--- Parámetros Cargados ---"
    print *, "Coriolis (f0): ", f0
    print *, "Escala Zonal (L_x): ", L_x
    print *, "Escala Meridional (L_y): ", L_y
    print *, "Amplitud: ", amp

    ! 2. Calcular números de onda y frecuencia angular (Relación de Dispersión)
    k = 2.0 * 3.14159265 / L_x
    l = 2.0 * 3.14159265 / L_y
    c = sqrt(g * H) ! Velocidad de la onda de gravedad somera
    omega = sqrt(f0**2 + (c**2) * (k**2 + l**2))
    
    print *, "Frecuencia de la Onda (omega): ", omega

    ! 3. Configurar la grilla espacial (Plano Infinito aproximado a 2 longitudes de onda)
    x_max = 2.0 * L_x
    y_max = 2.0 * L_y
    dx = x_max / real(N-1)
    dy = y_max / real(N-1)

    ! 4. Calcular y exportar los datos a un archivo
    !open(unit=20, file='poincare_data_eta_u_v_2H0.txt', status='unknown')
    open(unit=20, file=trim(output_txt_file), status='unknown')
    write(20, *) "# X(m) Y(m) Elevacion(eta) U(m/s) V(m/s)"

    do i = 0, N-1
        x = -L_x + real(i) * dx
        do j = 0, N-1
            y = -L_y + real(j) * dy
            
            ! Onda de Poincaré viajando en X (para t = 0)
            eta = amp * cos(k*x + l*y)
            
            ! Velocidades horizontales teóricas asociadas
            u_par = (amp * g / (omega**2 - f0**2)) * (k * omega * cos(k*x + l*y))
            u_per = (amp * g / (omega**2 - f0**2)) * (l * f0 * sin(k*x + l*y))

            u = ( k*u_par - l*u_per )/ sqrt(k**2 + l**2)
            v = ( l*u_par + k*u_per )/ sqrt(k**2 + l**2)
            
            write(20, '(5(E14.6, 1x))') x, y, eta, u, v
        end do
        write(20, *) ! Línea en blanco (útil para estructuras de datos de Gnuplot/Python)
    end do
    close(20)

    !print *, "Datos exportados exitosamente a: 'poincare_data_2H0.txt'"
    print *, "Datos exportados exitosamente a: ", trim(output_txt_file)
end program poincare_wave

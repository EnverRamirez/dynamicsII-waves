program kelvin_wave
    ...
    open(unit=20, file='kelvin_data.txt', status='unknown')
    ...
    do i = 0, N-1
        ...
        do j = 0, N-1
            ...
            eta = eta0 * exp(-f0*y/c0) * cos(k*x - omega*tt )
            
            ! Velocidades horizontales teóricas asociadas
            u = eta0 / H  * c0 * exp(-f0*y/c0) * cos(k*x - omega*tt )
            v = 0 
    ...
    print *, "Datos exportados exitosamente a 'kelvin_data.txt'"
end program kelvin_wave

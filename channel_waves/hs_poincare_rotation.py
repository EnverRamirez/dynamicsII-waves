...
nn = 3               # número cuántico meridional
...
for t in tiempos:
    ...
    eta = fac * (np.cos(l*y) - L_y * f0 * k / nn / np.pi / omega * np.sin(l*y))*np.cos(fase) 
    u = fac / H * (c**2 * k / omega * np.cos(l*y) - L_y * f0 / nn / np.pi * np.sin(l*y))*np.cos(fase) 
    v = -fac / H * L_y / omega / nn / np.pi * ( f0**2 + (c*nn*np.pi/L_y)**2 ) * np.sin(l*y) * np.sin(fase)
...
def update(frame):
    ...
    eta_field = amp * (np.cos(l*Y) - L_y * f0 * k / nn / np.pi / omega * np.sin(l*Y))*np.cos(fase) 
...
ani.save('hs_poincare_rotation.gif', writer='pillow', fps=20)
